...
- 我后续追加的提示词，请你写到 `project/prompt_log.md`，方便追溯（不要再维护其他位置的副本）
- 程序自动跑出来的东西，请都放在 x- 开头的文件夹，这部分东西 agent 是不允许修改的，如 x-log/., x-data/.
- 一键启动/关闭脚本放在 `scripts/dev.sh`，能读取 `$ENV` 指定的环境变量文件（没有就用默认）
## 测试管理
- 端到端/回归测试统一放在 `regression_tests/` 目录，单元测试/局部测试由各自目录维护
## 数据管理
