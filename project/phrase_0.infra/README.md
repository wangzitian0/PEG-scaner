...
- Host shared regression tests (see `regression_tests/`) for infra-level health checks.
...
- `append_promot.md`: prompts impacting infra decisions (mirrored from `project/prompt_log.md`).

## Status

- **State:** Active
- **Scope Owner:** Codex (AI Agent)
- **Dependencies:** `schema/`, `nx.json`, `backend/project.json`, `mobile/project.json`, `regression_tests/`, and future shared libraries.

## Notes

- When schema files change, run the codegen target noted in `backend/project.json` and capture results in `x-log/`.
- Nx upgrades or new executors should be documented here before touching app-specific phases.
- Ping-pong verification (backend `/api/ping/` + mobile 1px status indicator) lives here because it validates the infra stack end-to-end before higher-level features rely on it; the regression script is in `regression_tests/ping_pong.py`.
- Use `scripts/dev.sh start|stop` for a one-click lifecycle; it loads env vars from `$ENV` before launching backend/mobile services.
...
3. Add required targets (install/start/test/typecheck) immediately so `scripts/dev.sh` and CI can reference them.
4. Update `project/README.md` and the appropriate phase README to describe the new module.
