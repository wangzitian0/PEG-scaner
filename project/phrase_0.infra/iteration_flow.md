...
1. **Pre-work Checklist**
   - Read `AGENTS.md`, `docs/AI_EVALUATION.md`, `project/prompt_log.md`.
   - Review open items in `checklist.md` (proto/Nx tasks).

2. **Execution Loop**
   - For proto changes: update `schema/*.proto`, run `nx run backend:generate-proto`, inspect outputs under `backend/stock_research/generated`.
...
3. **Documentation Sweep**
   ...
   - Reflect any impacts in higher-level READMEs (project/, docs/, root).

4. **Handoff**
   - Note remaining risks or TODOs in `checklist.md`.
   - Ensure `project/prompt_log.md` includes any new prompts before closing the session.
