...
用 Git + Docker Compose 管结构，用 Vault 管理敏感环境变量/密钥，让 5 套环境（dev/ci/test/staging/prod）都能一键起、自动化部署，同时保持配置只有一个真理源。

**核心原则：**
- 代码库是公开 monorepo，包含应用代码和一套通用的 Docker Compose 模板
- 不包含任何真实密钥
- 所有敏感配置和各环境差异都集中放在自托管的 Vault 中
- dev/ci 要一键跑起来
...
应用只通过环境变量读取配置，不关心这些变量从哪里来，从而让"谁提供变量"这件事完全交给 Vault 和启动脚本。

**配置来源：**
- **本地/CI**：可以用默认值或从 Vault 导出的 `.env` 文件（可选）
- **test/staging/prod**：强制从 Vault 注入/读取当前版本，保证所有运行时配置都来自同一个中心

> [!IMPORTANT]
> 所有环境的配置都必须经过 Vault，确保版本历史和回滚能力 [3]
...
graph LR
    A[开发者] --> B[运行脚本]
    B --> C[从 Vault<br/>导出 .env]
    C --> D[启动 Docker<br/>Compose]
    D --> E[本地全栈<br/>就绪]
```

- 脚本先从 Vault 导出 dev 环境变量到 `.env`
...
- 通过 Vault 提供的正式连接串连到共享数据库和 Neo4j
- 最严格的变更流程
- 完整的回滚机制
...
| 组件 | 选型 | 理由 |
|-----|------|------|
| **配置管理** | Vault (自托管) | Policy/RBAC、审计、KV 版本、可注入到运行时 |
