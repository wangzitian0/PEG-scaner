# BRN-004: 环境即服务(EaaS)基础设施设计理念

> **文档类型:** Business Requirements & Design Rationale  
> **版本:** 1.0  
> **最后更新:** 2025-12-02  
> **相关文档:** [IRD-004](../specs/infra/IRD-004.env_eaas_infra.md) | [TRD-004](../specs/tech/TRD-004.env_eaas_implementation.md)

## 概述

本文档阐述了基础架构技术选型的核心理念和决策依据。整体架构围绕**「开源、自托管、单人强控、长期可扩展」**四个核心约束,选择最小但有成长空间的技术组合。

> 更新说明：运行时从 Dokploy 调整为 **k3s + Kubero + Kubero UI**，其余选型不变。落地实现以 infra 仓库的 `docs/BRN-004.env_eaas_design.md` 为准。
...
| **运行时编排** | k3s + Kubero (+ Kubero UI) | Dokploy, Docker Swarm | 保持单 VPS 简洁同时获得原生 k8s 生态与 PaaS UI |
...
### 2. k3s + Kubero（k8s 运行时 + PaaS UI）

**定位角色**

轻量级 k8s（k3s）作为运行时底座，配合 Kubero + Kubero UI 提供 GitOps/PaaS 体验（应用编排、证书/域名、部署流水线）。

**为什么选择 k3s + Kubero**

保留原生 k8s 生态与 CRD 能力，同时在单 VPS 场景保持安装/维护简洁；Kubero 提供 UI 和流水线，降低日常操作门槛。

**核心优势**

| 优势维度 | 具体内容 |
|---------|---------|
| **原生 k8s** | 直接兼容 Helm/CRD/Operator，可平滑扩到多节点 |
| **单机友好** | k3s 资源占用低，单 VPS 即可跑 |
| **PaaS 能力** | Kubero UI 提供应用模板、Git 集成、部署回滚、证书/域名管理 |
| **GitOps** | Kubero 支持 Git 驱动部署，结合 Terraform/Actions 易于自动化 |

**主要限制**

| 限制维度 | 具体内容 |
|---------|---------|
| **运维复杂度** | 需要额外的 k8s 心智模型（Helm/Ingress/Storage） |
| **资源占用** | k3s + Kubero 比单纯 Docker Stack 略高，需注意小机型内存 |
| **生态集中度** | Kubero 生态较小，部分插件/场景需自定义实现 |

> [!TIP]
> Kubero 负责 PaaS 与 UI，k3s 负责运行时；Terraform 仍是唯一的 IaC 入口。
...
"配置与环境全家桶" 的入口: 服务目录、环境目录、资源目录 + 自助模板动作,作为对接 Terraform、k3s/Kubero、Cloudflare 等的 orchestrator。

**为什么选择 Backstage 而不是 OpenChoreo**

已经选好了底座 (k3s+Kubero、Terraform、SigNoz、Cloudflare),需要的是一个高度可编程的 Portal,而不是再来一套 opinionated 平台骨架。Backstage 是开源领域唯一成熟的通用开发者门户框架。
...
| **模式契合** | 与 "Portal 驱动 Terraform/Kubero/CI" 的模式天然契合,<br>只需编写少量插件和模板 | [[26]][26], [[27]][27] |
...
| **插件覆盖** | 对 Cloudflare、Kubero 等没有官方插件,<br>需要通过 Terraform/CI 或编写自定义插件 | - |
...
| 优势维度 | 具体内容 |
|---------|---------|
| **开源自托管** | 能在 k3s/Kubero/VPS 或独立机器上运行,数据完全控制在自己手中 |
...
| **Secrets 管理** | Vault | 在 "单人、极强控制" 前提下，运行时密钥收口 Vault；bootstrap 仍用 infra tfvars/GitHub Secrets（trust anchor） | [[31]][31], [[38]][38] |
...
### Kubero / k3s
- [7]: [Kubero Official Site][7]
- [8]: [Kubero GitHub][8]
- [9]: [Kubero Docs - Getting Started][9]
...
[7]: https://kubero.dev
[8]: https://github.com/kubero-dev/kubero
[9]: https://docs.kubero.dev
[13]: https://signoz.io
