...
## 概述
构建 dev/ci/test/staging/prod 五套环境的一键启动与部署机制，使用 Docker Compose 分层结构 + Infisical 管理环境变量，保持配置单一真理源且无敏感信息入仓。
...
## 当前状态
🚧 进行中 — 已有 Compose 与 Infisical 方向，但具体执行记录缺失，需要补全指令与任务分解。

## 验收标准（待细化）
- dev/ci/test/staging/prod 可通过统一脚本/命令一键启动。
- 所有环境变量通过 Infisical 导出或注入，仓库不含真实密钥。
