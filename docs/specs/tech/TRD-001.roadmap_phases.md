...
> 
> **与 project/BRN-001/README.md 的关系**：本文件是规划，`project/BRN-001/README.md` 是执行状态
...
**Goal:** Establish the core monorepo structure, define essential data models using GraphQL (SDL), and set up basic data acquisition mechanisms.

**Key Deliverables:**
*   Initialized Nx workspace (completed, as per `agent.md` setup).
*   Defined core GraphQL schema (`schema.graphql`) for `Stock`, `KLineData`, `CompanyInfo`, `Financials`, etc. (Next Step).
