...
> **版本:** 1.0  
> **最后更新:** 2025-12-02  
> **相关文档:** [BRN-004](../../origin/BRN-004.dev_test_prod_design.md) | [IRD-004](../infra/IRD-004.env_eaas_infra.md)

## 概述

本文档概述基础架构的核心实施步骤和关键关注点，重点在于**理解实施思路**，具体配置细节在实际编码时再深入。

> 更新说明：运行时从 Dokploy 调整为 **k3s + Kubero + Kubero UI**，IaC 仍为 Terraform + GitHub Actions。
...
## 三、运行时层：k3s + Kubero 编排容器

### 核心目标
在 VPS 上安装 k3s 作为轻量级 Kubernetes，部署 Kubero/Kubero UI 提供 PaaS + GitOps 体验。

### 实施思路

```mermaid
graph TB
    A[SSH 到 VPS] --> B[安装 k3s]
    B --> C[部署 Kubero + Kubero UI]
    C --> D[配置 Ingress/证书]
    D --> E[接入 Git 仓库<br/>启用 GitOps 部署]
```

**k3s + Kubero 核心能力：**

- **原生 k8s 生态**：兼容 Helm/CRD/Operator，可平滑扩展
- **PaaS/UI**：Kubero 提供应用模板、流水线、证书/域名管理
- **GitOps**：跟踪 Git 变更自动部署，支持回滚
- **单机友好**：k3s 资源占用低，单 VPS 即可运行

**关键关注点：**

1. **单机起步**：初期所有服务（业务 + DB + 监控）都跑在同一个 VPS
2. **网络/Ingress**：使用 k3s 自带或自选 Ingress（Traefik/nginx），Kubero 负责路由/证书
3. **GitOps 配置**：应用 manifest/Helm values 提交到 infra 仓库，由 Kubero 监听并同步
...
### 核心目标
将应用构建为 Docker 镜像，通过 Kubero 部署到 k3s。
...
sequenceDiagram
    ...
    participant Kub as Kubero
    
    Dev->>App: 1. 提交代码
    App->>CI: 2. 触发 CI
    CI->>Reg: 3. 构建并推送镜像
    Dev->>Infra: 4. 更新镜像版本
    Infra->>Kub: 5. Git 触发 Kubero GitOps
    Kub->>Reg: 6. 拉取新镜像
    Kub->>Kub: 7. 滚动更新
```
...
- **Infra 仓库**：k8s manifest / Helm values（镜像 tag、env、ingress）

**关键关注点：**

1. **两仓库分离**：app 仓库可公开，infra 仓库私有保存配置和密钥
2. **版本管理**：通过镜像 tag（如 `v1.2.3`）控制部署版本
3. **环境变量**：敏感配置（DB 密码等）由 k8s Secret/Kubero 管理，不写入代码

**部署流程本质：**
```
代码变更 → CI 构建镜像 → 更新 infra 配置 → Kubero 自动部署
```
...
```mermaid
graph TB
    A[部署 SigNoz<br/>on k3s/Kubero] --> B[应用集成<br/>OpenTelemetry]
    B --> C[数据上报到<br/>SigNoz]
...
### Q1: Terraform 和 k3s/Kubero 的职责边界是什么？

**Terraform**：管理"有什么资源"（VPS、DNS、网络、k3s 安装）  
**k3s/Kubero**：管理"资源上跑什么服务"（k8s workload、应用）
...
# 方式2: 通过 Kubero UI 手动回滚
# 选择服务 → Deployments → 选择历史版本 → Rollback
```

### Q5: 故障排查第一步是什么？

1. 查看 k3s 节点/Pod：`kubectl get nodes,pods -A`
2. 查看应用日志：`kubectl logs deploy/api`
...
1. ✅ **理解架构**：回顾 [IRD-004](../infra/IRD-004.env_eaas_infra.md) 巩固整体认知
2. ✅ **准备环境**：注册账号、生成密钥
