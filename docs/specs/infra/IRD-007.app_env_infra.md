...
### SSOT 原则
- 应用只读 `ENV`，不关心来源
- Vault 按环境分路径前缀：`dev/` `ci/` `test/` `staging/` `prod/`
- 本地开发用默认值，无需 Vault 也能跑

### 敏感变量清单
| 变量 | 环境差异 |
|------|----------|
| `NEO4J_PASSWORD` | dev 用默认值，prod 从 Vault |
...
### 首次部署
```
1. Vault 配置 → 2. 数据库 (neo4j, postgres) → 3. backend → 4. cms → 5. frontend
```
...
| 类型 | 方式 |
|------|------|
| 配置回滚 | Vault KV 版本回滚（读取指定 version 并回写） |
